...
with st.sidebar:
    st.markdown(
        ...
        "1. Choose data sources.\n"
        "2. If CSV is chosen as a data source, upload a CSV file.\n"
        "3. Ask a question about the flights.\n"
    )
    st.markdown("---")
    st.markdown("# About")
    st.markdown(
        "AI app to find real-time flight details from various airlines in a specific location. "
        "It uses Pathway’s [LLM App features](https://github.com/pathwaycom/llm-app) "
        "to build a real-time LLM(Large Language Model)-enabled data pipeline in Python and joins data from multiple input sources.\n"
    )
...
# Paths for data files
amadeus_path = "../data/amadeus_flight_details.jsonl"
csv_path = "../data/csv_flight_details.jsonl"
...
# Streamlit UI elements
st.title("✈️ Flight Details Tracker with LLM App")
data_sources = st.multiselect(
    'Choose data sources',
    [source.value for source in DataSource]
)
...
question = st.text_input(
    "Search for a flight",
    placeholder="What flight details are you looking for?",
    disabled=not data_sources
)

# Handle data sources
if DataSource.AMADEUS_API.value in data_sources:
    shutil.copy("../amadeus/amadeus_flight_details.jsonl", amadeus_path)
elif os.path.exists(amadeus_path):
    os.remove(amadeus_path)

if DataSource.CSV.value not in data_sources and os.path.exists(csv_path):
    os.remove(csv_path)

# Handle Flight Details API request if data source is selected and a question is provided
if data_sources and question:
    if not os.path.exists(csv_path) and not os.path.exists(amadeus_path):
        st.error("Failed to process flight details file")
    ...
    if response.status_code == 200:
        ...
        st.error(f"Failed to send data to Flight Details API. Status code: {response.status_code}")
